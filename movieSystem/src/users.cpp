...
{
	nlohmann::json data;
	if (!Utilities::isFileEmpty(fileName)) {
		data = Utilities::loadFile(fileName);
	}
...
nlohmann::json User::saveAsJson() {
	...
	data["password"] = Utilities::sha256FromString(this->password);
...
bool User::loadFromFile(const std::string& fileName, const std::string& emailToFind)
{
	...
	for (const auto& item : data) {
		...
		if (storedEmail == emailToFind ||
			Utilities::sha256FromString(emailToFind) == storedEmail) {
